...
function updateTable(data) {
   let tableBody = document.querySelector('#atList4 .tbody1');
    ...
    for (let i = 0; i < data.length; i++) {
       ...
       if(record.egrade == 0){egrade = "사원";} 
        else if(record.egrade == 1){egrade = "주임";} 
        else if(record.egrade == 2){egrade = "대리";} 
        else if(record.egrade == 3){egrade = "과장";} 
        else if(record.egrade == 4){egrade = "차장";} 
        else if(record.egrade == 5){egrade = "부장";} 
        else if(record.egrade == 6){egrade = "부사장";}
        else if(record.egrade == 7){egrade = "사장";}
        else {egrade = "관리자";}
       
       if(record.atmgsts == 0){atmgsts = "정상출근";}
       else if(record.atmgsts == 1){atmgsts = "지각";} 
        else if(record.atmgsts == 2){atmgsts = "조퇴";} 
        else if(record.atmgsts == 3){atmgsts = "결근";} 
        else {atmgsts = "휴가";}
        ...
        cell1.textContent = record.rowNum;
        ...
        cell2.textContent = record.ename;
        newRow.appendChild(cell2);

        let cell3 = document.createElement('td');
        cell3.className = 'td1';
        cell3.style.width = '100px';
        cell3.style.minWidth = '100px';
        cell3.textContent = record.edept;
        newRow.appendChild(cell3);
        
        let cell4 = document.createElement('td');
        cell4.className = 'td1';
        cell4.style.width = '100px';
        cell4.style.minWidth = '100px';
        cell4.textContent = egrade;
        newRow.appendChild(cell4);
        
        let cell5 = document.createElement('td');
        cell5.className = 'td1';
        cell5.style.width = '200px';
        cell5.style.minWidth = '200px';        
        let date = new Date(record.atmgdate);
        ...
        cell6.className = 'td1';
        cell6.style.width = '200px';
        cell6.style.minWidth = '200px';
        cell6.textContent = record.atmgstr;
        newRow.appendChild(cell6);
        
        let cell7 = document.createElement('td');
        cell7.className = 'td1';
        cell7.style.width = '200px';
        cell7.style.minWidth = '200px';
        cell7.textContent = record.atmgend;
        newRow.appendChild(cell7);
        
        let cell8 = document.createElement('td');
        cell8.className = 'td1';
        cell8.textContent = atmgsts;
        newRow.appendChild(cell8);
        
        tableBody.appendChild(newRow);
    }
}
function sendPage(page2){
   $.ajax({
        url : "./attendAdmin",
        type : "post",
        data : {page2:page2},
        ...
        error : function(request, status, error) {alert("에러");}
   });
}
   ...
   <div class="div-btn">
    ...
    <c:choose>
      ...
      <c:otherwise>
         ...
         <div class="atList2" id="atList4">
         <div class="atList5"></div>
      <c:choose>
      <c:when test="${calList.size() eq 0 }"><h1>출결 내역이 없습니다.</h1></c:when>
      <c:otherwise>
            <table class="table1">
               <tbody>
                  <tr class="tr1">
                     <th class="th1" style="width: 100px; min-width: 100px;">관리번호</th>
                     <th class="th1" style="width: 100px; min-width: 100px;">이름</th>
                     <th class="th1" style="width: 100px; min-width: 100px;">부서</th>
                     <th class="th1" style="width: 100px; min-width: 100px;">직급</th>
                     <th class="th1" style="width: 200px; min-width: 200px;">출근일자</th>
                     <th class="th1" style="width: 200px; min-width: 200px;">출근시간</th>
                     <th class="th1" style="width: 200px; min-width: 200px;">퇴근시간</th>
                     <th class="th1">출결상태</th>
                  </tr>
               </tbody>
               <tbody class="tbody1">
                  <c:forEach items="${calList }" var="l" varStatus="status">
                     <tr class="tr1">
                        <td class="td1" style="width: 100px; min-width: 100px;">${fn:substring(l.rowNum,0,2)}</td>
                        <td class="td1" style="width: 100px; min-width: 100px;">${l.ename }</td>
                        <td class="td1" style="width: 100px; min-width: 100px;">${l.edept }</td>
                        <td class="td1" style="width: 100px; min-width: 100px;"><c:if
                              test="${l.egrade eq 0 }">사원</c:if> <c:if
                              test="${l.egrade eq 1 }">주임</c:if> <c:if
                              test="${l.egrade eq 2 }">대리</c:if> <c:if
                              test="${l.egrade eq 3 }">과장</c:if> <c:if
                              ...
                              test="${l.egrade eq 7 }">사장</c:if>
                           <c:if test="${l.egrade eq 8 }">관리자</c:if></td>
                        <td class="td1" style="width: 200px; min-width: 200px;">${l.atmgdate }</td>
                        <td class="td1" style="width: 200px; min-width: 200px;">${l.atmgstr }</td>
                        <td class="td1" style="width: 200px; min-width: 200px;">${l.atmgend }</td>
                        <td class="td1"><c:if test="${l.atmgsts eq 0 }">정상출근</c:if><c:if test="${l.atmgsts eq 1 }">지각</c:if><c:if test="${l.atmgsts eq 2 }">조퇴</c:if><c:if test="${l.atmgsts eq 3 }">결근</c:if><c:if test="${l.atmgsts eq 4 }">휴가</c:if></td></tr>
                  </c:forEach></tbody></table></c:otherwise></c:choose>
            <div class="page_wrap3">
           <div class="page_nation">
           <div class="page_left_wrap">
               <c:if test="${ph2.showPrev}">
                  <button class="page" onclick="location.href='./attendAdmin?page2=${ph2.startPage-1}'">이전</button>
               </c:if></div>
               <c:forEach var="i" begin="${ph2.startPage}" end="${ph2.endPage}">
                  <a class="page-b" onclick="sendPage(${i})">${i}</a>
               </c:forEach>
               <c:if test="${ph2.showNext}">
                  <button class="page" onclick="location.href='./attendAdmin?page2=${ph2.endPage+1}'">다음</button>
               </c:if>
            </div></div>
      </div>
