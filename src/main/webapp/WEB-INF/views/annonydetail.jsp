...
<body>
	...
	<article id="article">
      ...
      <div id="detail">
         ...
         <br>
  <c:if test="${sessionScope.eno == detail.eno || sessionScope.egrade == 8}">
					...
					<c:forEach items="${commentList }" var="row">
						<div class="divTableBody">
							<div class="divTableRow">
								<c:choose>
									<c:when
										test="${row.csecret == 0 && (sessionScope.eno == detail.eno|| sessionScope.eno == row.eno || sessionScope.egrade == 8)}">
										...
										<div class="divTableCell">${row.ccomment}<div style="font-weight: bold; display: inline; color: red;">
											...
											<c:if test="${sessionScope.eno == row.eno || sessionScope.egrade == 8}">
		...
		$(function() {
			...
			$(".cedit").click(function() {
				...
				//글씨 상자 만들기
		    	  ...
		    	  recommentBox += '<textarea type="text" id = "rcta" name = "comment"  style="width: 80%; resize: none; color:red; " placeholder="댓글을 입력하세요" maxlength = 50;>'+content+'</textarea>';
		    	  ...
		    	  recommentBox += '<button type="submit" id="recomment" style="margin-left:113px; margin-bottom:14px;">수정</button>';
		    	 recommentBox += '</form>';
		    	  recommentBox += '</div>';
				
		    	  $(document).on("click", "#recomment", function(){
		    		    if($("#rcta").val().length < 5){
		    		    	alert("댓글을 5글자 이상 입력해주세요.");
		    		    	$("#rcta").focus();
		    		    	return false;
		    		    }
